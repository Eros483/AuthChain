...
export default function HomePage() {
  ...
  return (
    <main
      ref={containerRef}
      className="relative bg-[#0B0E14] text-[#F3F4F6] h-[400vh] antialiased selection:bg-[#4DA3FF]/30"
    >
      {/* ADMIN LINK */}
      <div className="fixed top-6 right-8 z-50">
        <Link
          href="/admin"
          className="text-xs uppercase tracking-widest text-[#9BA3B4] hover:text-[#E6E8EB] transition-colors"
        >
          Admin
        </Link>
      </div>
      ...
      <div className="sticky top-0 h-screen w-full overflow-hidden">
        ...
        >
          ...
          <p className="max-w-xl text-center text-base sm:text-lg text-[#9BA3B4] font-light leading-relaxed">
            Deterministic agent execution enforced by{" "}
            <span className="text-white">on-chain governance</span> and irreversible
            state transitions.
          </p>
          ...
          <div className="mt-12">
            <Link
              href="/chat"
              className="group inline-flex items-center gap-3 rounded-full bg-[#E6E8EB] px-10 py-4 text-sm font-semibold text-[#0B0E14] transition-all hover:bg-white hover:scale-[1.03]"
            >
        ...
        >
          <div className="relative w-full h-full max-w-7xl">
            <div className="absolute inset-0 z-20 flex flex-col items-center justify-center text-center pointer-events-none">
              <span className="text-[#4DA3FF] text-[10px] font-bold tracking-[0.4em] uppercase mb-6 opacity-80">
                Current Limitations
              </span>
              <h2 className="text-5xl sm:text-8xl font-medium tracking-tight">
            ...
            {referenceData.map((ref, i) => (
              ...
              >
                <div
                  className={`${ref.size} group relative overflow-hidden rounded-xl border border-white/10 bg-[#121826] shadow-2xl transition-transform duration-500 hover:scale-110 hover:z-50 hover:border-[#4DA3FF]/50`}
                >
                  <Image
                    src={ref.image}
                    alt={ref.label}
